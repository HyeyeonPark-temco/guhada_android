...
import io.temco.guhada.databinding.ItemCommonhorizontalImageBinding
import io.temco.guhada.view.holder.base.BaseViewHolder

class CommonImageAdapter : RecyclerView.Adapter<CommonImageAdapter.Holder>() {
    var mList: MutableList<String> = arrayListOf()

    var mClickSelectItemListener : OnClickSelectItemListener? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder =
            Holder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_commonhorizontal_image, parent, false))
    ...
    inner class Holder(binding: ItemCommonhorizontalImageBinding) : BaseViewHolder<ItemCommonhorizontalImageBinding>(binding.root) {
        fun bind(photo: String, position: Int) {
            setSpacing()
            mBinding.imagePath = photo
