 ...
 */
class KidsListAdapter(private val model : KidsListViewModel, list : ArrayList<MainBaseModel>) :
        CommonRecyclerAdapter<MainBaseModel, KidsListAdapter.ListViewHolder>(list){
    /**
    ...
    override fun setOnBindViewHolder(viewHolder: ListViewHolder, item: MainBaseModel, position: Int) {
        viewHolder.bind(model as KidsListViewModel, position, item)
    }
    ...
    open abstract class ListViewHolder(containerView: View, binding: ViewDataBinding) : BaseProductViewHolder<ViewDataBinding>(containerView){
        abstract fun bind(viewModel : KidsListViewModel, position : Int, item : MainBaseModel)
    }
    ...
    class SubTitleLayoutViewHolder(private val containerView: View, val binding: CustomlayoutMainItemSubtitleBinding) : ListViewHolder(containerView, binding){
        ...
        override fun init(context: Context?, manager: RequestManager?, data: Deal?, position : Int) { }
        override fun bind(model: KidsListViewModel, position: Int, item: MainBaseModel) {
    ...
    inner class SubTitleViewHolder(private val containerView: View, val binding: CustomlayoutMainItemSubtitlelistBinding) : ListViewHolder(containerView, binding){
        ...
        override fun bind(viewModel: KidsListViewModel, position: Int, item: MainBaseModel) {
            if(item is SubTitleItemList){
                ...
                val model : KidsListViewModel = this@KidsListAdapter.model as KidsListViewModel
                ...
                for (i in 0..5){
                    ...
                    if(data != null){
                        ...
                        // Ship
                        //if(CustomLog.flag)CustomLog.L("KidsListAdapter",item.title,"SubTitleViewHolder textShipFree","data.freeShipping",data.freeShipping)
