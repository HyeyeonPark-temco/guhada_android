...
class WomenListAdapter(private val model : ViewModel, list : ArrayList<MainBaseModel>) :
    ...
    override fun setOnBindViewHolder(viewHolder: ListViewHolder, item: MainBaseModel, position: Int) {
        viewHolder.bind(model as WomenListViewModel, position, item)
    }
    ...
    open abstract class ListViewHolder(containerView: View, binding: ViewDataBinding) : BaseProductViewHolder<ViewDataBinding>(containerView){
        abstract fun bind(viewModel : WomenListViewModel, position : Int, item : MainBaseModel)
    }
    ...
    inner class SubTitleViewHolder(private val containerView: View, val binding: CustomlayoutMainItemSubtitlelistBinding) : ListViewHolder(containerView, binding){
        ...
        override fun bind(viewModel: WomenListViewModel, position: Int, item: MainBaseModel) {
            if(item is SubTitleItemList){
                ...
                val model : WomenListViewModel = this@WomenListAdapter.model as WomenListViewModel
                ...
                for (i in 0..size){
                    ...
                    if(data != null){
                        ...
                        // Ship
                        //if(CustomLog.flag)CustomLog.L("WomenListAdapter",item.title,"SubTitleViewHolder textShipFree","data.freeShipping",data.freeShipping)
