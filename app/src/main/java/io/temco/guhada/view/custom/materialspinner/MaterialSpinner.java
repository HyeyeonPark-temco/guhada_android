...
public class MaterialSpinner extends AppCompatTextView {
  ...
  private boolean startIndexHalf;
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    boolean rtl = MaterialSpinnerUtils.isRtl(context);
    ...
    try {
      backgroundColor = ta.getResourceId(R.styleable.MaterialSpinner_ms_background_color, 0);
      ...
      arrowColor = ta.getColor(R.styleable.MaterialSpinner_ms_arrow_tint, hintColor);
      hideArrow = ta.getBoolean(R.styleable.MaterialSpinner_ms_hide_arrow, false);
      hintText = ta.getString(R.styleable.MaterialSpinner_ms_hint) == null ? context.getString(R.string.common_select_spinner) : ta.getString(R.styleable.MaterialSpinner_ms_hint);
      ...
      arrowColorDisabled = MaterialSpinnerUtils.lighter(arrowColor, 0.8f);
      startIndex = ta.getInteger(R.styleable.MaterialSpinner_ms_start_index, -1);
      textSize = ta.getInteger(R.styleable.MaterialSpinner_ms_text_size, 13);
      startIndexHalf = ta.getBoolean(R.styleable.MaterialSpinner_ms_start_index_half, false);
    } finally {
      ta.recycle();
    }
    ...
    if (!hideArrow) {
      arrowDrawable = MaterialSpinnerUtils.getDrawable(context, R.drawable.ms__arrow).mutate();
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      //popupWindow.setBackgroundDrawable(MaterialSpinnerUtils.getDrawable(context, R.drawable.ms__drawable));//new ColorDrawable(Color.WHITE)
      popupWindow.setBackgroundDrawable(MaterialSpinnerUtils.getDrawable(context, R.drawable.border_all_whitethree));//new ColorDrawable(Color.WHITE)
    } else {
      //popupWindow.setBackgroundDrawable(MaterialSpinnerUtils.getDrawable(context, R.drawable.ms__drawable));//ms__drop_down_shadow
      popupWindow.setBackgroundDrawable(MaterialSpinnerUtils.getDrawable(context, R.drawable.border_all_whitethree));//new ColorDrawable(Color.WHITE)
    }

    if (backgroundColor != 0) { // default color is white
  ...
  @Override public void setBackgroundColor(int color) {
    ...
    if (background instanceof StateListDrawable) { // pre-L
      try {
        ...
        int[] colors = { MaterialSpinnerUtils.darker(color, 0.85f), color };
  ...
  public <T> void setItems(@NonNull List<T> items) {
    ...
    if(nothingSelected && (startIndexHalf || startIndex != -1)){
      if(startIndexHalf){
        listView.setSelection(items.size()/2-1);
      }else{
